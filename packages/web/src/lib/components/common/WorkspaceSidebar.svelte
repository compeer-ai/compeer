<script lang="ts">
  ...
  const { stores, workspaces, user }: Props = $props();
  const workspace = page.params.workspace;
  function isActiveProject(storeId: string) {
    return (
      page.url.pathname.startsWith(`/${workspace}/store/${storeId}`) &&
      page.params.storeId === storeId
    );
  }
